...
/**
 * A Vector and its set of k nearest neighbours.
 * KNNVector is a vector and has some Vectors.
 * THere are 4 methods by which the contentss of a KNNVectors neighbours can ebe modified:
 * build_knn_graph(), update_knn(), set_k(), reset_k().
 * An exception is thrown if you try to access a neighbour that has not been set.
 * to set nodes have to call build_knn_graph().
 */
class KNNVector
{
	const EMPTY_LABEL = 0;
 ...
 */	
	...
	/**
	 * Removes a fn call to update in building a graph in this way.
	 * @1000pts this is worth about a 0.8s.
	 * timings @1000pts
	 * 	baseline 				0.01s;
	 *    + for for nothing  	0.3s;
	 * 	+ dist calc				2.6s;
	 * 	+ for k nothing		6.6s;
	 * 	+ everything			6.0s; (break);
	 *		+ ins zeros				4.7s;
	 * 	
	 */
	public static function build_knn_graph( Array $nodes )
	{
		...
		{
			$curr = $nodes[$i];			
			for( $j = $i+1; $j < $num_nodes; $j += 1 )
			{
				$vec = $nodes[$j];
				$dist = $curr->vector->distance( $vec->vector );

				//scan knn. Note -1 trick.
				for( $m = $k_end; $m >= -1 ; $m -= 1 )
				{				
					if( $dist >= $curr->dist[$m] )
					{
    ...
    * THIS RELIES ON THE INPUT DATA BEING SORTED.
	 ...
	 * Arrays are filled to onde greater than their size.
	 ...
	 * -1 is set to a value <= to any possible dist to a neigb
	 * also for optimization used in build_knn_graph().
	 */
